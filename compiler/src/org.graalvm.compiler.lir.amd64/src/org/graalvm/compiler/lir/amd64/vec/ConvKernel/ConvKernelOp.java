...
public final class ConvKernelOp extends AMD64LIRInstruction {
    public static final LIRInstructionClass<ConvKernelOp> TYPE = LIRInstructionClass.create(ConvKernelOp.class);
    ...
    public ConvKernelOp(LIRGeneratorTool tool, Value arrs, Value kPanelSize,
    ...
    public void emitCode(CompilationResultBuilder crb, AMD64MacroAssembler masm) {
        ...
        masm.movq(imgReg, new AMD64Address(arrsPtr, loopIndex, OBJECT_ARRAY_INDEX_SCALE, OBJECT_ARRAY_BASE_OFFSET));
        masm.movq(kernelReg, new AMD64Address(arrsPtr, loopIndex, OBJECT_ARRAY_INDEX_SCALE, OBJECT_ARRAY_BASE_OFFSET+8));
        ...
        masm.movq(rax, new AMD64Address(imgReg, kernelOutChannelReg, OBJECT_ARRAY_INDEX_SCALE, OBJECT_ARRAY_BASE_OFFSET));
        ...
        for(int j = 0; j < bLength; j++) {
            masm.vmovupd(xmmRegistersAVX512[simdRegisters.get("B" + String.valueOf(j))], new AMD64Address(rax, rdx, DOUBLE_ARRAY_INDEX_SCALE, DOUBLE_ARRAY_BASE_OFFSET+(j*64)));
        }

        for(int i = 0; i < aLength; i++) {
            masm.movq(rax, new AMD64Address(kernelReg, iPos, OBJECT_ARRAY_INDEX_SCALE, OBJECT_ARRAY_BASE_OFFSET+(i*8)));
            masm.movq(rax, new AMD64Address(rax, kernelOutChannelReg, OBJECT_ARRAY_INDEX_SCALE, OBJECT_ARRAY_BASE_OFFSET));
            masm.movq(rax, new AMD64Address(rax, colOffsetReg, OBJECT_ARRAY_INDEX_SCALE, OBJECT_ARRAY_BASE_OFFSET));
            masm.vbroadcastsd(xmmRegistersAVX512[simdRegisters.get("A")], new AMD64Address(rax, rowOffsetReg, DOUBLE_ARRAY_INDEX_SCALE, DOUBLE_ARRAY_BASE_OFFSET));
            for(int j = 0; j < bLength; j++) {
                ...
                //masm.vfmadd231pd(xmmRegistersAVX512[simdRegisters.get("C" + String.valueOf(i) + String.valueOf(j))], xmmRegistersAVX512[simdRegisters.get("A")], xmmRegistersAVX512[simdRegisters.get("A")]);
                //masm.vfmadd231pd(xmmRegistersAVX512[simdRegisters.get("C" + String.valueOf(i) + String.valueOf(j))], xmmRegistersAVX512[simdRegisters.get("B" + String.valueOf(j))], xmmRegistersAVX512[simdRegisters.get("B" + String.valueOf(j))]);
                //masm.vmovupd(xmmRegistersAVX512[simdRegisters.get("C" + String.valueOf(i) + String.valueOf(j))], xmmRegistersAVX512[simdRegisters.get("A")]);
            }
        }
        ...
        */
        ...
        masm.movq(loopIndex, 0);
        masm.movq(resultPtr, new AMD64Address(arrsPtr, loopIndex, OBJECT_ARRAY_INDEX_SCALE, OBJECT_ARRAY_BASE_OFFSET+16));
        ...
        for(int i = 0; i < aLength; i++) {
            masm.movq(resultIindexed, new AMD64Address(resultPtr, iPosTemp, OBJECT_ARRAY_INDEX_SCALE, OBJECT_ARRAY_BASE_OFFSET+(i*8)));
            for(int j = 0; j < bLength; j++) {
                masm.movq(rdx, 0);
                masm.movq(rax, 0);
                ...
                masm.addq(rax, 8*j);
                AMD64Assembler.AMD64MOp.IDIV.emit(masm, QWORD, outLengthReg);
                masm.movq(addressTempReg, new AMD64Address(resultIindexed, rax, OBJECT_ARRAY_INDEX_SCALE, OBJECT_ARRAY_BASE_OFFSET));
                resultAddress = new AMD64Address(addressTempReg, rdx, DOUBLE_ARRAY_INDEX_SCALE, DOUBLE_ARRAY_BASE_OFFSET);
    ...
    protected static void debugPrint(String msg) {
        ...
        try{
            debugLog = new PrintWriter(new FileWriter("/home/junyoung2/project/project2/adaptive-code-generation/log.txt", true));
        } catch (Exception e) {
