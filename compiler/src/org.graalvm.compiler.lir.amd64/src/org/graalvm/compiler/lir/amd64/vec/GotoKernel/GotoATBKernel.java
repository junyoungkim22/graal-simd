...
public final class GotoATBKernel extends GotoKernel {
  ...
  public void subIter(
    ...
    if (prefetchDistance > 0) {
      bAddress =
          new AMD64Address(
              ...
              OBJECT_ARRAY_BASE_OFFSET + (offset * 8) + (prefetchDistance * 8));
    ...
    bAddress =
        new AMD64Address(
            bPtr, loopIndex, OBJECT_ARRAY_INDEX_SCALE, OBJECT_ARRAY_BASE_OFFSET + (offset * 8));
    masm.movq(tempArrayAddressReg, bAddress);
    if (interleave) {
    ...
    aAddress =
        new AMD64Address(
            aPtr, loopIndex, OBJECT_ARRAY_INDEX_SCALE, OBJECT_ARRAY_BASE_OFFSET + (offset * 8));
    masm.movq(tempArrayAddressReg, aAddress);
    ...
    if (interleave) {
      for (int i = 0; i < aLength; i += 2) {
        ...
        for (int j = 0; j < bLength * 2; j++) {
          ...
          // Register cRegister = xmmRegistersAVX512[simdRegisters.get("C" + String.valueOf(i+(j%2))
          // + String.valueOf(j/2))];
        ...
        */
        ...
        masm.movq(
            ...
            new AMD64Address(
                ...
                iPos,
                OBJECT_ARRAY_INDEX_SCALE,
                OBJECT_ARRAY_BASE_OFFSET + (iIndex * 8)));
        int j = jIndex;
        masm.vmovddup(
            xmmRegistersAVX512[dstRegNum],
            new AMD64Address(
      ...
      for (int i = 0; i < aLength; i += 2) {
        ...
        masm.movq(
            ...
            new AMD64Address(
                ...
                iPos,
                OBJECT_ARRAY_INDEX_SCALE,
                OBJECT_ARRAY_BASE_OFFSET + ((i + 1) * 8)));
        for (int j = 0; j < bLength * 2; j++) {
          if (j % 2 == 0) {
            masm.vunpcklpd(
