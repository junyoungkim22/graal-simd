...
public final class GotoABKernel extends GotoKernel {
  ...
  public void subIter(
    ...
    if (prefetchDistance > 0) {
      bAddress =
          new AMD64Address(
              loopIndex, OBJECT_ARRAY_BASE_OFFSET + (offset * 8) + (prefetchDistance * 8));
  ...
  protected void loadA(AMD64MacroAssembler masm, int iIndex, int offset, int dstRegNum) {
    ...
    if (iIndex < aTempArrayAddressNumLimit) {
      ...
      masm.movq(
          tempArrayAddressReg, new AMD64Address(rsp, 8 * (iIndex - aTempArrayAddressNumLimit)));
      aAddress =
          new AMD64Address(
              tempArrayAddressReg,
              loopIndex,
  ...
  protected void loadB(AMD64MacroAssembler masm, int jIndex, int offset, int dstRegNum) {
    ...
    bAddress =
        new AMD64Address(
            ...
            jPos,
            DOUBLE_ARRAY_INDEX_SCALE,
            DOUBLE_ARRAY_BASE_OFFSET + (jIndex * 64));
  ...
  protected void emitKernelCode(AMD64MacroAssembler masm, int aLength, int bLength) {
    ...
    masm.movq(
        loopIndex,
        new AMD64Address(rsp, (numOfAAddressOnStack * 8) + (useAsAddressRegs.length * 8)));
    ...
    if (transposeResult) {
      ...
      for (int i = 0; i < aLength; i++) {
        for (int j = 0; j < bLength; j++) {
          resultAddress =
              new AMD64Address(
                  iPos, xmmRegistersAVX512[0 + j], AMD64Address.Scale.Times1, 12 + (i * 4));
          masm.kxnorw(k2, k2, k2);
          masm.vpgatherqq(xmmRegistersAVX512[31], k2, resultAddress);
          masm.vaddpd(
              xmmRegistersAVX512[simdRegisters.get("C" + String.valueOf(i) + String.valueOf(j))],
              xmmRegistersAVX512[simdRegisters.get("C" + String.valueOf(i) + String.valueOf(j))],
              xmmRegistersAVX512[31]);
          masm.kxnorw(k2, k2, k2);
          masm.vpscatterqq(
              resultAddress,
              k2,
