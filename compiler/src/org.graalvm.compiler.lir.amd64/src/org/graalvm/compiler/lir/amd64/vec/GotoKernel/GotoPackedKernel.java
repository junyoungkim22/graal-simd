...
public final class GotoPackedKernel extends GotoKernel {
    ...
    public void subIter(int aLength, int bLength, int offset, int prefetchDistance, AMD64MacroAssembler masm, Map<String, Integer> simdRegisters, 
                        Register aPtr, Register bPtr, Register aIndex, Register bIndex) {
        ...
        if(interleave) {
            if(varArgProperties[argIndex] == 2) {
                ...
                */
                ...
                masm.movq(zeroReg, 0);
                masm.movq(zeroReg, new AMD64Address(arrsPtr, zeroReg, OBJECT_ARRAY_INDEX_SCALE, OBJECT_ARRAY_BASE_OFFSET+24+8*argIndex));
                masm.movq(zeroReg, new AMD64Address(zeroReg, iPos, OBJECT_ARRAY_INDEX_SCALE, OBJECT_ARRAY_BASE_OFFSET+(iIndex*8)));
                int j = jIndex;
                masm.vmovddup(xmmRegistersAVX512[dstRegNum], new AMD64Address(zeroReg, jPos, DOUBLE_ARRAY_INDEX_SCALE, DOUBLE_ARRAY_BASE_OFFSET+((j/2)*64)+((j%2)*8)));
        ...
        */
        ...
        for(int i = 0; i < varArgProperties.length; i++) {
            if(varArgProperties[i] == 2) {
                ...
                genRegCountdown++; // Todo: Do something when genRegCountdown == remainingRegisterNum
                masm.movq(varArgRegister, new AMD64Address(arrsPtr, temp, OBJECT_ARRAY_INDEX_SCALE, OBJECT_ARRAY_BASE_OFFSET+24+8*i));
                varArgAddresses.put(i, varArgRegister);
            }
        }
        availableGenRegs.offer(temp);
        ...
        for(int i = 0; i < unrollFactor; i++) {
            subIter(aLength, bLength, i, prefetchDistance, masm, simdRegisters, aPtr, bPtr, aIndex, bIndex);
        }
