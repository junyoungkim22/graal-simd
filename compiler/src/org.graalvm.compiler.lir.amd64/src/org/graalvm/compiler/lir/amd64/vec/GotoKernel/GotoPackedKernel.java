...
public final class GotoPackedKernel extends GotoKernel {
    ...
    public GotoPackedKernel(LIRGeneratorTool tool, int kernelType, int aLength, int bLength, int mLength, int kLength, int nLength,
                        long[] calc, double[] constArgs, int[] varArgProperties, GotoKernelOp kernelOp, int kPack) {
    ...
    public void subIter(int aLength, int bLength, int offset, int prefetchDistance, AMD64MacroAssembler masm, Map<String, Integer> simdRegisters, 
        ...
        for(int i = 0; i < aLength; i++) {
            aAddress = new AMD64Address(aPtr, aIndex, DOUBLE_ARRAY_INDEX_SCALE, DOUBLE_ARRAY_BASE_OFFSET+(i*8) - aAddressOffset);
            ...
            for(int j = 0; j < bLength; j++) {
                ...
                //masm.vmovupd(xmmRegistersAVX512[simdRegisters.get("C" + String.valueOf(i) + String.valueOf(j))], xmmRegistersAVX512[simdRegisters.get("B" + String.valueOf(j))]);
            }
        }
    }
